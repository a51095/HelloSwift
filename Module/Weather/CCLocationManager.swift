...
struct CCWeatherModel: HandyJSON {
    /// 室外温度
    var temp: String?
    /// 体感温度
...
class CCLocationManager: NSObject, CLLocationManagerDelegate {
    private static let obj = CCLocationManager()
    private static let manager = CLLocationManager()
    public static var location: CLLocationCoordinate2D?
    private static var weatherHandle: ((CCWeatherModel) -> Void)?
    ...
    /// 获取天气信息
    static func requestWeather(_ res: @escaping ((CCWeatherModel) -> Void))  {
        ...
        AF.request(url, method: .get, parameters: param).responseJSON { responseData in
            ...
            let model = CCWeatherModel.deserialize(from: now)
            // 非空校验
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        CCLocationManager.location = coordinate
    }
}
